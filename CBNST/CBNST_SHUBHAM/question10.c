...
int main()
{
    ...
    int n;
    printf("Enter the number of observation you want to enter : ");
    scanf("%d",&n);
    int observation[n][n+1];
    printf("Enter the observations \n");
    for(int i=0;i<n;i++){
        scanf("%d",&observation[i][0]);
        scanf("%d",&observation[i][1]);
    }
    ...
    for(int i=0;i<n;i++){
        for(int j=0;j<n+1-i;j++)  printf("%d ",observation[i][j]);
        printf("\n");
    }
    ...
    scanf("%d",&x);
    double p = (x-observation[0][0])/(double)(observation[1][0]-observation[0][0]);
    ...
    printf("The solution for the observations on the value of x is %lf",ans);
    return 0;
}