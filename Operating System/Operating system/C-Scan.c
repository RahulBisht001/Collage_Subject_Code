...
void customC_SCAN(int requestSequence[], int initialHead, char moveDirection[], int totalTracks, int requestCount)
{
    int totalSeekTime = 0;
    ...
    while (runs--)
    {
        ...
        {
            for (int i = ((runs == 0) ? 0 : leftCount - 1); ((runs == 0) ? (i < leftCount) : (i >= 0)); ((runs == 0) ? (i++) : (i--)))
            {
        ...
        {
            for (int i = ((runs == 0) ? rightCount - 1 : 0); ((runs == 0) ? (i >= 0) : (i < rightCount)); ((runs == 0) ? (i--) : (i++)))
            {
...
int main()
{
    ...
    customC_SCAN(requests, initialHead, moveDirection, totalTracks, totalRequests);
    printf("\n\n");
    return 0;
}
